...
func TestNewThreadSafeOrderedSet(t *testing.T) {
	t.Parallel()

	t.Run("wraps existing ordered set", func(t *testing.T) {
		t.Parallel()

		s := set.NewOrderedSet[hashing.HashableString](hashing.Sha256)
		tsos := set.NewThreadSafeOrderedSet(s)
		require.NotNil(t, tsos)
		assert.Equal(t, 0, tsos.Size())
	})
	...
	t.Run("wrapped set is usable immediately", func(t *testing.T) {
		...
		s := set.NewOrderedSet[hashing.HashableString](hashing.Sha256)
		tsos := set.NewThreadSafeOrderedSet(s)
		err := tsos.Add(hashing.HashableString("test"))
		require.NoError(t, err)
		assert.Equal(t, 1, tsos.Size())
	})
	...
	t.Run("returns existing thread-safe ordered set as-is", func(t *testing.T) {
		t.Parallel()

		s := set.NewOrderedSet[hashing.HashableString](hashing.Sha256)
		tsos1 := set.NewThreadSafeOrderedSet(s)
		tsos2 := set.NewThreadSafeOrderedSet(tsos1)

		// Should be the same instance, not double-wrapped
		assert.Equal(t, fmt.Sprintf("%p", tsos1), fmt.Sprintf("%p", tsos2))
	})
}

//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_Add(t *testing.T) {
	t.Parallel()

	t.Run("adds new element", func(t *testing.T) {
		t.Parallel()

		s := set.NewThreadSafeOrderedSet(set.NewOrderedSet[hashing.HashableString](hashing.Sha256))
	...
	t.Run("maintains insertion order", func(t *testing.T) {
		...
		for _, elem := range elements {
			err := s.Add(hashing.HashableString(elem))
			require.NoError(t, err)
		}

		entries := s.Entries()
		for i, elem := range elements {
			assert.Equal(t, hashing.HashableString(elem), entries[i])
		}
	})
	...
	t.Run("concurrent adds are safe", func(t *testing.T) {
		...
		threadSafeSet := set.NewThreadSafeOrderedSet(set.NewOrderedSet[hashing.HashableString](hashing.Sha256))
		numGoroutines := 10
...
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_AddAll(t *testing.T) {
	t.Parallel()
	...
	t.Run("adds multiple elements in order", func(t *testing.T) {
		...
		s := set.NewThreadSafeOrderedSet(set.NewOrderedSet[hashing.HashableString](hashing.Sha256))
		elements := []hashing.HashableString{
			"elem1",
			"elem2",
			"elem3",
		}
...
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_Remove(t *testing.T) {
	t.Parallel()
	...
	t.Run("maintains order after removal", func(t *testing.T) {
		...
		s := set.NewThreadSafeOrderedSet(set.NewOrderedSet[hashing.HashableString](hashing.Sha256))
		elements := []hashing.HashableString{"first", "second", "third", "fourth"}
		...
		err := s.Remove(hashing.HashableString("second"))
		require.NoError(t, err)

		entries := s.Entries()
		expected := []hashing.HashableString{"first", "third", "fourth"}
...
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_Clear(t *testing.T) {
	t.Parallel()
...
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_Contains(t *testing.T) {
	t.Parallel()
...
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_Size(t *testing.T) {
	t.Parallel()
...
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_Entries(t *testing.T) {
	t.Parallel()
	...
	t.Run("returns all elements in insertion order", func(t *testing.T) {
		...
		s := set.NewThreadSafeOrderedSet(set.NewOrderedSet[hashing.HashableString](hashing.Sha256))
		expected := []hashing.HashableString{"first", "second", "third"}
...
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_Seq(t *testing.T) {
	t.Parallel()
...
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_Union(t *testing.T) {
	t.Parallel()
	...
	t.Run("combines two sets maintaining order", func(t *testing.T) {
		...
		// Verify order: s1 elements first, then s2 elements
		entries := result.Entries()
		expected := []hashing.HashableString{"elem1", "elem2", "elem3", "elem4"}
		assert.Equal(t, expected, entries)
	})
	...
	t.Run("concurrent union operations are safe", func(t *testing.T) {
		...
		firstSet := set.NewThreadSafeOrderedSet(set.NewOrderedSet[hashing.HashableString](hashing.Sha256))
		...
		for opIndex := range numOperations {
			...
			go func(index int) {
				...
				secondSet := set.NewOrderedSet[hashing.HashableString](hashing.Sha256)
...
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_Intersection(t *testing.T) {
	t.Parallel()

	t.Run("returns common elements maintaining order", func(t *testing.T) {
		t.Parallel()
		...
		// Verify order preserved from first set
		entries := result.Entries()
		expected := []hashing.HashableString{"elem2", "elem3"}
		assert.Equal(t, expected, entries)
	})
...
// TestThreadSafeOrderedSet_RaceConditions uses go test -race to detect race conditions.
//
//nolint:dupl // Intentional duplication with thread_safe_test.go for parallel test coverage
func TestThreadSafeOrderedSet_RaceConditions(t *testing.T) {
	t.Parallel()
